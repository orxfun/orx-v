...
pub trait NVecCore<D: Dim, T>: Sized {
    ...
    fn core_child(&self, i: D::ChildIdx) -> impl NVecCore<D::PrevDim, T>;
...
impl<D: Dim, V: NVecCore<D, T>, T> NVecCore<D, T> for &V {
    fn core_num_children(&self) -> usize {
        <V as NVecCore<D, T>>::core_num_children(self)
    }

    fn core_card(&self, idx: impl Into<<D as Dim>::CardIdx>) -> usize {
        <V as NVecCore<D, T>>::core_card(self, idx)
    }

    fn core_child(&self, i: <D as Dim>::ChildIdx) -> impl NVecCore<<D as Dim>::PrevDim, T> {
        <V as NVecCore<D, T>>::core_child(self, i)
    }

    fn core_map<F: FnMut(&T) -> O, O>(&self, idx: impl IntoIdx<D>, f: &mut F) -> O {
        <V as NVecCore<D, T>>::core_map(self, idx, f)
    }

    fn core_is_rectangular(&self) -> bool {
        <V as NVecCore<D, T>>::core_is_rectangular(self)
    }
}

impl<D: Dim, V: NVecCore<D, T>, T> NVecCore<D, T> for &mut V {
    fn core_num_children(&self) -> usize {
        <V as NVecCore<D, T>>::core_num_children(self)
    }
